function main(requestURL) {
    ...
    requestURL += '?JusteVedette=' + document.getElementsByName('OnlyVedette')[0].checked

    for (var rdi of document.getElementsByName('MaxVedette')) {
        if (rdi.checked) {
            requestURL += '&MaxVedette=' + rdi.value;
        }
    }
    ...
    function MaFonctionCallBack() {
        ...
        if (!document.getElementsByName('OnlyVedette')[0].checked) { afficheBoxOffice(Film); }
    ...
    function ConstruireEntete(jsonObj) {
        ...
        var Nsection = section.length;

        for (var i = Nsection - 1; i >= 0 ; i--) {
        ...
        for (var i = 0; i < Films.length; i++) {
            ...
            if (!document.getElementsByName('OnlyVedette')[0].checked) {
